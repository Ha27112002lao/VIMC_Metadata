...
- By default, after importing Glossary terms, the script creates/updates a technical
  Database asset displayed as "VIMC Technical term" and publishes precomputed
  Data Quality test results to Observability > Data Quality.
- Use --skip-data-quality if you only want the old Glossary import behavior.
"""
...
# -----------------------------
# OpenMetadata requires Data Quality test cases to be attached to a data asset.
# The script therefore creates a lightweight technical Database hierarchy and a
# synthetic metadata Table that represents the imported glossary rows.
DEFAULT_DQ_DATABASE_DISPLAY = "VIMC Technical term"
DEFAULT_DQ_SERVICE = "VIMC_Technical_term_Service"
DEFAULT_DQ_SERVICE_TYPE = "CustomDatabase"
DEFAULT_DQ_DATABASE = "VIMC_Technical_term"
DEFAULT_DQ_SCHEMA = "glossary_quality"
DEFAULT_DQ_TABLE = "imported_glossary_terms"
DEFAULT_DQ_TEST_SUITE = "VIMC_Technical_term_DQ"

DQ_TABLE_COLUMNS: Tuple[Dict[str, str], ...] = (
    {"name": "source_file", "dataType": "STRING", "description": "Tên file Excel nguồn."},
    {"name": "sheet_name", "dataType": "STRING", "description": "Tên sheet Excel nguồn."},
    {"name": "excel_row", "dataType": "INT", "description": "Dòng Excel gốc."},
    {"name": "import_order", "dataType": "INT", "description": "Thứ tự import trong sheet."},
    {"name": "group_name", "dataType": "STRING", "description": "Nhóm/parent của thuật ngữ."},
    {"name": "code", "dataType": "STRING", "description": "Mã thuật ngữ/chỉ tiêu nếu có."},
    {"name": "term_name", "dataType": "STRING", "description": "Tên term gốc nếu có."},
    {"name": "display_name", "dataType": "STRING", "description": "Tên hiển thị được đưa lên Glossary."},
    {"name": "definition", "dataType": "STRING", "description": "Định nghĩa/mô tả nghiệp vụ."},
    {"name": "source_key", "dataType": "STRING", "description": "Khóa kỹ thuật tạo từ file/sheet/dòng/order."},
    {"name": "glossary_name", "dataType": "STRING", "description": "Glossary đích trong OpenMetadata."},
    {"name": "imported_at", "dataType": "TIMESTAMP", "description": "Thời điểm script publish Data Quality result."},
)
...
class DQMetric:
    ...
    test_definition: str
    entity_link: str
    parameter_values: Tuple[Dict[str, Any], ...]
    status: str
    result: str
    result_values: Tuple[Dict[str, str], ...]
...
class OpenMetadataClient:
    ...
    def get_by_name(self, path_prefix: str, fqn: str) -> Optional[dict]:
        """Read an entity by FQN/name and return None when it does not exist."""
        encoded = quote(fqn, safe="")
        url = f"{self.base_url}{path_prefix}/name/{encoded}"
        response = self.session.get(url, timeout=self.timeout)
        if response.status_code == 404:
            return None
        if response.status_code >= 400:
            raise RuntimeError(
                f"\nAPI ERROR {response.status_code} GET {url}\n"
    ...
    def upsert_database_service(
        ...
        payload = {
            ...
            "description": (
                "Technical database service created by the VIMC glossary import script "
                "to host Data Quality observability metadata."
            ),
        }
        if dry_run:
            print(f"[DRY-RUN] Upsert database service: {name} ({service_type})")
            return {"name": name, "fullyQualifiedName": name}
        result = self.put("/services/databaseServices", payload)
        print(f"✅ Upsert database service: {name}")
        return result

    def upsert_database_asset(
        self,
        name: str,
        display_name: str,
        service_fqn: str,
        ...
        payload = {
            ...
            "service": service_fqn,
            "description": (
                "Technical database created automatically for VIMC Glossary import "
                "Data Quality checks. It is metadata-only unless you connect it to a real source."
            ),
        }
        if dry_run:
            print(f"[DRY-RUN] Upsert database: {service_fqn}.{name} displayName='{display_name}'")
            return {"name": name, "fullyQualifiedName": fqn_join(service_fqn, name)}
        result = self.put("/databases", payload)
        print(f"✅ Upsert database: {service_fqn}.{name}")
        return result

    def upsert_database_schema_asset(
        self,
        name: str,
        database_fqn: str,
        dry_run: bool = False,
        ...
        payload = {
            ...
            "description": "Schema containing the technical table used for glossary Data Quality checks.",
        }
        if dry_run:
            print(f"[DRY-RUN] Upsert database schema: {database_fqn}.{name}")
            return {"name": name, "fullyQualifiedName": fqn_join(database_fqn, name)}
        result = self.put("/databaseSchemas", payload)
        print(f"✅ Upsert database schema: {database_fqn}.{name}")
        return result

    def upsert_table_asset(
        self,
        name: str,
        display_name: str,
        ...
        payload = {
            ...
            "tableType": "Regular",
            "description": (
                "Synthetic metadata table representing rows imported from Excel into the VIMC Glossary. "
                "The script computes Data Quality metrics from Excel and publishes the results here."
            ),
            "columns": list(columns),
        }
        if dry_run:
            print(f"[DRY-RUN] Upsert table: {database_schema_fqn}.{name}")
            return {"name": name, "fullyQualifiedName": fqn_join(database_schema_fqn, name)}
        result = self.put("/tables", payload)
        print(f"✅ Upsert table: {database_schema_fqn}.{name}")
        return result

    def ensure_dq_asset_hierarchy(
        self,
        service_name: str,
        service_type: str,
        database_name: str,
        ...
        service = self.upsert_database_service(
            name=service_name,
            display_name=f"{database_display_name} Service",
            service_type=service_type,
            dry_run=dry_run,
        )
        service_fqn = service.get("fullyQualifiedName") or service_name
        database = self.upsert_database_asset(
            name=database_name,
            display_name=database_display_name,
            service_fqn=service_fqn,
            dry_run=dry_run,
        )
        database_fqn = database.get("fullyQualifiedName") or fqn_join(service_fqn, database_name)
        schema = self.upsert_database_schema_asset(
            name=schema_name,
            database_fqn=database_fqn,
            dry_run=dry_run,
        )
        schema_fqn = schema.get("fullyQualifiedName") or fqn_join(database_fqn, schema_name)
        table = self.upsert_table_asset(
            name=table_name,
            display_name="Imported glossary terms",
            database_schema_fqn=schema_fqn,
            columns=DQ_TABLE_COLUMNS,
            dry_run=dry_run,
        )
        return table.get("fullyQualifiedName") or fqn_join(schema_fqn, table_name)

    def create_or_get_executable_test_suite(
        self,
        suite_name: str,
        table_fqn: str,
        dry_run: bool = False,
        ...
        if dry_run:
            print(f"[DRY-RUN] Create/get executable test suite: {suite_name} for {table_fqn}")
            return {"id": suite_name, "name": suite_name, "fullyQualifiedName": suite_name}

        existing = self.get_by_name("/dataQuality/testSuites", suite_name)
        if existing:
            print(f"✅ Reuse test suite: {suite_name}")
            return existing

        payload = {
            "name": suite_name,
            "displayName": "VIMC Technical term Data Quality",
            "description": "Executable test suite for VIMC Glossary import quality metrics.",
            "executableEntityReference": table_fqn,
        }
        result = self.request("POST", "/dataQuality/testSuites/executable", payload)
        print(f"✅ Create executable test suite: {suite_name}")
        return result

    def upsert_test_case(self, metric: DQMetric, test_suite_fqn: str, dry_run: bool = False) -> dict:
        payload = {
            "name": metric.name,
            "description": metric.description,
            "testDefinition": metric.test_definition,
            "testSuite": test_suite_fqn,
            "entityLink": metric.entity_link,
            "parameterValues": list(metric.parameter_values),
        }
        if dry_run:
            print(f"[DRY-RUN] Upsert test case: {metric.name} -> {metric.status}")
            return {"id": metric.name, "fullyQualifiedName": metric.name}
        result = self.put("/dataQuality/testCases", payload)
        print(f"✅ Upsert test case: {metric.name}")
        return result

    def add_test_case_result(self, test_case: dict, metric: DQMetric, dry_run: bool = False) -> None:
        timestamp_ms = int(time.time() * 1000)
        payload = {
            "timestamp": timestamp_ms,
            "testCaseStatus": metric.status,
            "result": metric.result,
            "testResultValue": list(metric.result_values),
        }
        target = test_case.get("id") or test_case.get("fullyQualifiedName") or metric.name
        path = f"/dataQuality/testCases/{quote(str(target), safe='')}/testCaseResult"
        if dry_run:
            print(f"[DRY-RUN] Publish result: {metric.name} = {metric.status} ({metric.result})")
            return
        self.put(path, payload)
        print(f"✅ Publish DQ result: {metric.name} = {metric.status}")
...
def collect_dq_rows_from_sheet(
    excel_path: Path,
    sheet_name: str,
    ws,
    glossary_name: str,
    ...
    _, _, _, field_to_col, _, rows = collect_sheet_rows(ws, header_row, data_start_row)
    result: List[DQImportedRow] = []
    for import_order, (excel_row, row_values) in enumerate(rows, start=1):
        code = mapped_value(row_values, field_to_col, "code")
        term_name_value = mapped_value(row_values, field_to_col, "term_name")
        indicator = mapped_value(row_values, field_to_col, "indicator")
        ...
        definition = mapped_value(row_values, field_to_col, "definition")
        group_value = mapped_value(row_values, field_to_col, "group") or ungrouped_label
        display_name = display_name_value or indicator_new or indicator or term_name_value or code
        source_key = f"{excel_path.name}|{sheet_name}|row:{excel_row}|order:{import_order}"
        result.append(
            DQImportedRow(
                source_file=excel_path.name,
                sheet_name=sheet_name,
...
def collect_dq_rows(
    ...
    all_rows: List[DQImportedRow] = []
    for excel_path in excel_paths:
        workbook = load_workbook(excel_path, data_only=True, read_only=False)
        try:
            for sheet_name in detect_best_sheet(workbook, requested_sheet, all_sheets):
                all_rows.extend(
                    collect_dq_rows_from_sheet(
                        excel_path=excel_path,
                        sheet_name=sheet_name,
                        ws=workbook[sheet_name],
                        glossary_name=glossary_name,
...
def dq_table_entity_link(table_fqn: str) -> str:
    return f"<#E::table::{table_fqn}>"


def dq_column_entity_link(table_fqn: str, column_name: str) -> str:
    return f"<#E::table::{table_fqn}::columns::{column_name}>"


def missing_count(rows: Sequence[DQImportedRow], field: str) -> int:
    return sum(1 for row in rows if not is_meaningful(getattr(row, field)))


def distinct_count(values: Sequence[str]) -> int:
    return len({normalize_text(value) for value in values if normalize_text(value)})


def evaluate_dq_metrics(rows: Sequence[DQImportedRow], table_fqn: str) -> List[DQMetric]:
    total_rows = len(rows)
    table_cols = {column["name"] for column in DQ_TABLE_COLUMNS}
    max_reasonable_rows = max(1000000, total_rows + 1000)

    metrics: List[DQMetric] = []

    metrics.append(
        DQMetric(
            name="vimc_row_count_positive",
            description="Số dòng glossary import phải lớn hơn 0.",
            test_definition="tableRowCountToBeBetween",
            entity_link=dq_table_entity_link(table_fqn),
            parameter_values=(
                {"name": "minValue", "value": "1"},
                {"name": "maxValue", "value": str(max_reasonable_rows)},
            ),
            status="Success" if 1 <= total_rows <= max_reasonable_rows else "Failed",
            result=f"Imported row count = {total_rows}; expected between 1 and {max_reasonable_rows}.",
            result_values=({"name": "rowCount", "value": str(total_rows)},),
        )
    )

    for col in ("source_file", "sheet_name", "display_name", "definition", "source_key"):
        exists = col in table_cols
        metrics.append(
            DQMetric(
                name=f"vimc_column_{col}_exists",
                description=f"Cột kỹ thuật bắt buộc '{col}' phải tồn tại trong table DQ.",
                test_definition="tableColumnNameToExist",
                entity_link=dq_table_entity_link(table_fqn),
                parameter_values=({"name": "columnName", "value": col},),
                status="Success" if exists else "Failed",
                result=f"Column '{col}' {'exists' if exists else 'does not exist'} in {DEFAULT_DQ_TABLE}.",
                result_values=({"name": "columnName", "value": col}, {"name": "exists", "value": str(exists)}),
            )
        )

    display_missing = missing_count(rows, "display_name")
    metrics.append(
        DQMetric(
            name="vimc_display_name_not_null",
            description="Tên hiển thị của thuật ngữ không được rỗng.",
            test_definition="columnValuesToBeNotNull",
            entity_link=dq_column_entity_link(table_fqn, "display_name"),
            parameter_values=({"name": "columnValuesToBeNotNull", "value": "true"},),
            status="Success" if display_missing == 0 else "Failed",
            result=f"display_name missing count = {display_missing}; expected 0.",
            result_values=({"name": "missingCount", "value": str(display_missing)}, {"name": "rowCount", "value": str(total_rows)}),
        )
    )

    definition_missing = missing_count(rows, "definition")
    metrics.append(
        DQMetric(
            name="vimc_definition_missing_count_eq_0",
            description="Định nghĩa/mô tả của thuật ngữ nên được điền đầy đủ.",
            test_definition="columnValuesMissingCountToBeEqual",
            entity_link=dq_column_entity_link(table_fqn, "definition"),
            parameter_values=(
                {"name": "missingValueMatch", "value": ["", "-", "--", "N/A", "NA", "null"]},
                {"name": "missingCountValue", "value": "0"},
            ),
            status="Success" if definition_missing == 0 else "Failed",
            result=f"definition missing count = {definition_missing}; expected 0.",
            result_values=({"name": "missingCount", "value": str(definition_missing)}, {"name": "rowCount", "value": str(total_rows)}),
        )
    )

    source_keys = [row.source_key for row in rows]
    duplicate_count = len(source_keys) - distinct_count(source_keys)
    metrics.append(
        DQMetric(
            name="vimc_source_key_unique",
            description="Mỗi dòng import phải có source_key duy nhất để truy vết về file/sheet/dòng Excel.",
            test_definition="columnValuesToBeUnique",
            entity_link=dq_column_entity_link(table_fqn, "source_key"),
            parameter_values=({"name": "columnNames", "value": "true"},),
            status="Success" if duplicate_count == 0 else "Failed",
            result=f"source_key duplicate count = {duplicate_count}; expected 0.",
            result_values=({"name": "duplicateCount", "value": str(duplicate_count)}, {"name": "distinctCount", "value": str(distinct_count(source_keys))}),
        )
    )

    display_lengths = [len(clean(row.display_name)) for row in rows if is_meaningful(row.display_name)]
    min_len = min(display_lengths) if display_lengths else 0
    max_len = max(display_lengths) if display_lengths else 0
    invalid_len = sum(1 for length in display_lengths if length < 1 or length > 255) + display_missing
    metrics.append(
        DQMetric(
            name="vimc_display_name_length_1_255",
            description="Độ dài display_name phải nằm trong khoảng 1-255 ký tự.",
            ...
            status="Success" if invalid_len == 0 else "Failed",
            result=f"display_name length min={min_len}, max={max_len}, invalid rows={invalid_len}; expected length 1-255.",
            result_values=(
                {"name": "minObservedLength", "value": str(min_len)},
                {"name": "maxObservedLength", "value": str(max_len)},
                {"name": "invalidRowCount", "value": str(invalid_len)},
            ),
        )
    )
...
def run_data_quality_publication(
    client: OpenMetadataClient,
    rows: Sequence[DQImportedRow],
    service_name: str,
    service_type: str,
    database_name: str,
    database_display_name: str,
    ...
    print("\n=== VIMC/OpenMetadata Data Quality ===")
    print(f"DQ Database displayName: {database_display_name}")
    print(f"DQ imported rows evaluated: {len(rows)}")

    table_fqn = client.ensure_dq_asset_hierarchy(
    ...
    suite = client.create_or_get_executable_test_suite(
        suite_name=test_suite_name,
        table_fqn=table_fqn,
        dry_run=dry_run,
    )
    suite_fqn = suite.get("fullyQualifiedName") or suite.get("name") or test_suite_name

    metrics = evaluate_dq_metrics(rows, table_fqn)
    success_count = 0
    failed_count = 0
    for metric in metrics:
        test_case = client.upsert_test_case(metric, suite_fqn, dry_run=dry_run)
        client.add_test_case_result(test_case, metric, dry_run=dry_run)
        if metric.status == "Success":
            success_count += 1
        else:
            failed_count += 1

    print(f"DQ summary: {success_count} Success, {failed_count} Failed")
...
def main() -> int:
    ...
    parser.add_argument(
        "--skip-data-quality",
        action="store_true",
        help="Giữ đúng hành vi cũ: chỉ import Glossary, không tạo Database và không publish Data Quality.",
    )
    parser.add_argument(
        "--dq-strict",
        action="store_true",
        help="Nếu Data Quality API lỗi thì trả exit code 1. Mặc định chỉ cảnh báo để không làm hỏng luồng import Glossary.",
    )
    parser.add_argument(
        "--dq-service-name",
        default=DEFAULT_DQ_SERVICE,
        help=f"Database Service kỹ thuật cho DQ. Default: {DEFAULT_DQ_SERVICE}",
    )
    parser.add_argument(
        "--dq-service-type",
        default=DEFAULT_DQ_SERVICE_TYPE,
        help=f"Service type khi tạo Database Service. Default: {DEFAULT_DQ_SERVICE_TYPE}",
    )
    parser.add_argument(
        "--dq-database-name",
        default=DEFAULT_DQ_DATABASE,
        help=f"Entity name của Database kỹ thuật. Default: {DEFAULT_DQ_DATABASE}",
    )
    parser.add_argument(
        "--dq-database-display",
        default=DEFAULT_DQ_DATABASE_DISPLAY,
        help=f"Display name của Database kỹ thuật trên UI. Default: {DEFAULT_DQ_DATABASE_DISPLAY}",
    )
    parser.add_argument(
        "--dq-schema",
        default=DEFAULT_DQ_SCHEMA,
        help=f"Schema kỹ thuật chứa table DQ. Default: {DEFAULT_DQ_SCHEMA}",
    )
    parser.add_argument(
        "--dq-table",
        default=DEFAULT_DQ_TABLE,
        help=f"Table kỹ thuật để gắn test cases. Default: {DEFAULT_DQ_TABLE}",
    )
    parser.add_argument(
        "--dq-test-suite",
        default=DEFAULT_DQ_TEST_SUITE,
        help=f"Tên Test Suite DQ. Default: {DEFAULT_DQ_TEST_SUITE}",
    )
    ...
    print(f"Data Quality: {'disabled' if args.skip_data_quality else 'enabled'}")
    if not args.skip_data_quality:
        print(f"DQ Database displayName: {args.dq_database_display}")
    print("Files:")
    ...
    if not args.skip_data_quality:
        try:
            ...
            run_data_quality_publication(
                client=client,
                rows=dq_rows,
                service_name=safe_name(args.dq_service_name, fallback=DEFAULT_DQ_SERVICE),
                service_type=args.dq_service_type,
                database_name=safe_name(args.dq_database_name, fallback=DEFAULT_DQ_DATABASE),
                database_display_name=args.dq_database_display,
                schema_name=safe_name(args.dq_schema, fallback=DEFAULT_DQ_SCHEMA),
                table_name=safe_name(args.dq_table, fallback=DEFAULT_DQ_TABLE),
                test_suite_name=safe_name(args.dq_test_suite, fallback=DEFAULT_DQ_TEST_SUITE),
                dry_run=args.dry_run,
            )
        except Exception as exc:
            print(f"\n⚠️ Glossary đã import xong nhưng Data Quality bị lỗi: {exc}")
            print("Gợi ý: nếu server OpenMetadata không hỗ trợ serviceType mặc định, thử --dq-service-type Postgres hoặc dùng service có sẵn rồi chỉnh payload theo connector thực tế.")
            if args.dq_strict:
                return 1

    if not args.dry_run:
        print(f"Kiểm tra Glossary trên OpenMetadata UI: Govern → Glossary → {glossary_name}")
        if not args.skip_data_quality:
            print(f"Kiểm tra Data Quality trên UI: Observability → Data Quality → Test Suites → {safe_name(args.dq_test_suite, fallback=DEFAULT_DQ_TEST_SUITE)}")
            print(f"Kiểm tra Database trên UI: Explore → Databases → {args.dq_database_display}")
    return 0
